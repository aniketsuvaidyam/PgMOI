...
const createBeneficiary = async (req, res) => {
    ...
    try {
        ...
        const benefi = await Beneficiary.findOne({ where: { mobile: mobile, username: username, created_by: created_by } })
        if (benefi) {
            res.status(400).json({ message: "Duplicate Beneficiary" })
        } else {

            const beneficary = await Beneficiary.create({ state, district, village, block, name, username, email, mobile, address, created_by })
